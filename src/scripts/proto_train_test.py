    ...
    for e in range(config['num_experiment_runs']):
        ...
        while True:
            ...
            if update % (total_number_of_patches//query_set.shape[0]) == 0:
                ...
                vis_vars = ['[epoch-eval-train]', False, True, True, False, model_folder]
                ...
                vis_vars = ['[epoch-eval-test]', False, True, True, False, model_folder]
                tf_vars = [sess, x, q, is_train, log_p_y, emb_q, emb_prototypes]
                _ = eval(config, ids_test, id2audio_repr_path, support_set, id2gt_test, id2label_test, tf_vars, vis_vars)

                #if epoch_acc == 1:
                #    print('Accuracy 1!')
                #    perfect_epoch += 1
                #    if perfect_epoch > config['max_accuracy']:
                #        break
