...
if __name__ == '__main__':
    ...
    for e in range(config['num_experiment_runs']):
        ...
        if config['model_number'] > 10:
            fy.write('Epoch\ttrain_cost\tval_cost\taccuracy\tepoch_time\tlearing_rate\talpha\tbeta\n')
        else:
        ...
        for i in range(config['epochs']):
            ...
            if config['model_number'] > 10:
                print(sess.run('model/log_learn/alpha:0'))
                print(sess.run('model/log_learn/beta:0'))

            # validation
            tf_vars = [sess, normalized_y, cost, x, y_, is_train]
            accuracy, val_cost = evaluation(val_batch_streamer, id2label_val, ids_val, tf_vars)
